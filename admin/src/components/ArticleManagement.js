...
function ArticleManagement() {
  ...
  const filteredArticles = articles
    .filter((article) => {
      ...
      const matchesCategory =
        filterCategory === "all" ||
        article.category === parseInt(filterCategory);
  ...
  {filteredArticles.map((article, index) => (
    <tr key={article.id || index}> {/* Fallback to index if ID is missing */}
      <td>{article.title}</td>
      <td>{getCategoryName(article.category)}</td>
      <td>{article.author}</td>
      <td className="datetime-cell">
        {article.publishedAt ? formatDateTime(article.publishedAt) : "N/A"}
      </td>
      <td>
      ...
      <td>
        <div className="action-buttons">
          <button className="view-btn" onClick={() => handleView(article.id)} title="View Article">
            <Eye size={16} />
          </button>
          <button className="edit-btn" onClick={() => handleEdit(article.id)} title="Edit Article">
            <Edit size={16} />
          </button>
          <button className="delete-btn" onClick={() => handleDelete(article.id)} title="Delete Article">
            <Trash2 size={16} />
          </button>
        </div>
      </td>
