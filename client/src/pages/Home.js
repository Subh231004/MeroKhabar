  ...
  useEffect(() => {
    fetch("http://localhost:3001/api/articles")
      ...
      .then((data) => {
        ...
        setLatestNews(data.sort((a, b) => new Date(b.publishedAt) - new Date(a.publishedAt)).slice(0, 5))
      })
  }, [])
          ...
          {featuredArticles[0] && (
            <div className="main-featured">
              <Link to={`/article/${featuredArticles[0].id}`}>
                <img src={featuredArticles[0].image || "/placeholder.svg"} alt={featuredArticles[0].title} />
            ...
            <div className="latest-news">
              {latestNews.map((article) => (
                <div key={article.id} className="latest-news-item">
                  <Link to={`/article/${article.id}`}>
                    <h4>{article.title}</h4>
                    <span className="time">{new Date(article.publishedAt).toLocaleTimeString()}</span>
                  </Link>
                </div>
              ))}
            </div>
